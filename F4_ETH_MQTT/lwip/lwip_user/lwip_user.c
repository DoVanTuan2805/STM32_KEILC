...
uint8_t localIP[4] = {192,168,1,12};
uint8_t gatewayIP[4] = {192,168,1,1};
...
char ETH_IPOK(void)
{
	if(lwip_netif.ip_addr.addr != 0)
	{
		...
		subnetmask[0]=lwip_netif.netmask.addr&0x000000FF;
		subnetmask[1]=(lwip_netif.netmask.addr>>8)&0x000000FF;
		subnetmask[2]=(lwip_netif.netmask.addr>>16)&0x000000FF;
		subnetmask[3]=(lwip_netif.netmask.addr>>24)&0x000000FF;
		
		gatewayIP[0]=	lwip_netif.gw.addr 				& 0x000000FF;
		gatewayIP[1]=	(lwip_netif.gw.addr>>8) 	& 0x000000FF;
		gatewayIP[2]=	(lwip_netif.gw.addr>>16) 	& 0x000000FF;
		gatewayIP[3]=	(lwip_netif.gw.addr>>24) 	& 0x000000FF;
		return 1;
	}
	else return 0;
}
