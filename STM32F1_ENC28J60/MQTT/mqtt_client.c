...
#define MQTT_TOPIC_MAX_NAME_LENGTH 10
...
static char m_mqtt_tx_buffer[100];		// 256
    ...
    */
    if (result != ERR_OK)
    {
        //DEBUG_INFO("Retry send subscribe...\r\n");
				//UART_putString("Retry send subscribe...\n");
        m_sub_req_err_count++;
        ...
        else
        {
            ...
            if (err != ERR_OK)
            {
                //DEBUG_INFO("mqtt_subscribe error %d\r\n", err);
								UART_putString("mqtt_subscribe error\n");
            }
        }
    }
    else
...
static void mqtt_incoming_data_cb(void *arg, const u8_t *data, u16_t len, u8_t flags)
{
    ...
    if (flags & MQTT_DATA_FLAG_LAST)
    {
        ...
        //DEBUG_INFO("Payload data: %s\r\n", (const char *)data);
				//sprintf(dataz, "Payload data: %s\r\n", (char*)data);
				UART_putString((char*)data);
...
static void mqtt_dns_found(const char *hostname, const ip_addr_t *ipaddr, void *arg)
{
    ...
    if (ipaddr != NULL)
    {
        ...
        UART_putString("Server address resolved\r\n");
				m_is_dns_resolved = 1;
        m_mqtt_process_now = true;
    }
    else
    {
        ...
        UART_putString("mqtt_dns_found: Failed to resolve server address resolved, trying next server\r\n");
				m_is_dns_resolved = 0;
    }
}
...
void mqtt_client_polling_task(void *arg)
{
    ...
    if (ETH_IPOK())			// get ip oke
    {
        if (m_mqtt_allow == false)
        {
            ...
            {
								UART_putString("MQTT STATE CONNECT 1\r\n");
                mqtt_disconnect(&m_mqtt_static_client);
                m_mqtt_state = MQTT_CLIENT_STATE_DISCONNECTED;
            ...
            //DEBUG_INFO("MQTT thread started\r\n");
						UART_putString("MQTT thread started\r\n");
        }
    }
    else
    {
        if (m_mqtt_allow)
        {
            //DEBUG_INFO("MQTT thread stopped\r\n");
						UART_putString("MQTT thread stopped\r\n");
    ...
    if (m_mqtt_allow)
    {
        ...
        case MQTT_CLIENT_STATE_DISCONNECTED:
            ...
            mqtt_tick = 4;
						UART_putString("MQTT STATE DISCONNECT \r\n");
            break;
...
//                                    err,
//                                    m_cfg.broker_addr);
													UART_putString("Waiting for server\r\n");
										}
                    else if (err == ERR_OK)
                    {
...
//                                   m_cfg.broker_addr,
//                                   ipaddr_ntoa(&m_mqtt_server_address));
												UART_putString("DNS resolved aready\r\n");
                        m_is_dns_resolved = 1;
                    }
                }
            }
        ...
        case MQTT_CLIENT_STATE_CONNECTED:
        {
            ...
            if (mqtt_client_is_connected(&m_mqtt_static_client))
            {
                ...
                if (tick - last_time_send_hearbeat >= (uint32_t)3000)
                {
...
void mqtt_client_initialize(mqtt_client_cfg_t *cfg)
{
    ...
    snprintf(m_mqtt_publish_topic_name, sizeof(m_mqtt_publish_topic_name), "%s", "pub_stm32");
    snprintf(m_mqtt_subscribe_topic_name, sizeof(m_mqtt_subscribe_topic_name), "%s", "sub_stm32");
		UART_putString("MQTT client init\r\n");
//		UART_putString((char*)m_cfg.broker_addr);
//		uint8_t data1[10];
//		sprintf((char*)data1, "%d\n", m_cfg.port);
//		UART_putString((char*)data1);
		//HAL_UART_Transmit(&huart1, (uint8_t*)m_cfg.port, sizeof(m_cfg.port), 1000);
}
